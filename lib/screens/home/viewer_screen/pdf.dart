...
class _PdfViewerScreenState extends State<PdfViewerScreen> {
  ...
  try {
    debugPrint('Starting _openInBrowser...');
    ...
    debugPrint('Final URL to open: $urlToOpen');
    ...
    try {
      ...
      debugPrint('Invalid URL format: $urlToOpen');
      throw Exception('Invalid URL format: $e');
    }
    
    // Check if URL has proper scheme
    if (!uri.hasScheme || (uri.scheme != 'http' && uri.scheme != 'https')) {
      debugPrint('URL missing scheme or invalid scheme: ${uri.scheme}');
      // Try adding https:// if no scheme
      if (!uri.hasScheme) {
        urlToOpen = 'https://$urlToOpen';
        uri = Uri.parse(urlToOpen);
        debugPrint('Added https scheme: $urlToOpen');
      } else {
    ...
    if (canLaunch) {
      ...
      // Try different launch modes based on platform
      ...
      debugPrint('Using launch mode: $launchMode');
      ...
      if (!launched) {
        throw Exception('Failed to launch URL even though canLaunchUrl returned true');
      } else {
        debugPrint('Successfully launched URL in browser');
        
        // Show success message
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Opening PDF in browser...'),
      ...
      if (!kIsWeb && (Platform.isAndroid || Platform.isIOS)) {
        debugPrint('Trying alternative launch method for mobile...');
        
        // For mobile, try with different parameters
        bool altLaunched = await launchUrl(
          uri,
          mode: LaunchMode.externalNonBrowserApplication,
        ).catchError((error) async {
          debugPrint('Alternative launch failed: $error');
          // Final fallback - try platform default
          return await launchUrl(uri, mode: LaunchMode.platformDefault);
        });
        
        if (altLaunched) {
          debugPrint('Alternative launch succeeded');
          return;
        }
      }
      
      throw Exception('Cannot launch URL: $urlToOpen\nPlease check if you have a web browser installed.');
    }
    
  } catch (e, stackTrace) {
    debugPrint('Error in _openInBrowser: $e');
    debugPrint('Stack trace: $stackTrace');
    ...
    if (mounted) {
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: const Text('Error Opening PDF'),
          content: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text('Failed to open PDF in browser:'),
                ...
                const Text('• Copy the URL and paste it in your browser'),
                const Text('• Check if you have a web browser installed'),
                const Text('• Try downloading the PDF instead'),
                const SizedBox(height: 16),
                SelectableText(
                  'URL: ${widget.url}',
                  style: const TextStyle(
